...
test.describe("homepage OpenGraph card", () => {
  ...
  test("names the settled positioning, not a stale role string", async ({
    page,
  }) => {
    const { alt } = await import("@/app/opengraph-image");

    expect(alt).toContain(RESUME_DATA.positioning.label);
    expect(alt).not.toContain(RESUME_DATA.about);
