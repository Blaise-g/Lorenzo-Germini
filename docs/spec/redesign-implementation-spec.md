...
3. **Primary-action treatment — LOCKED.** The subscribe submit becomes the _only_ filled control in the system: **solid accent ground, 12px uppercase, ≥44px tall**. Every other CTA keeps the 11px accent underline. ~~Both were specified as mono.~~ (Both are the shared label face since ADR-0006 — the lock is on the _treatment_, and the control still matches the CTAs it must outrank.) Rationale: #13 measured 11 discrete CTAs in the last ~600px of `/writing`, 10 sharing one treatment — the submit was indistinguishable from a navigation link. A ~44px control is not a "large fill" and does not violate #8's accent discipline.
...
**Typography:** add Fraunces (variable, `opsz` axis, normal + italic) — ~~the only font addition~~ (Space Grotesk is a second, for the label role — ADR-0006). Inter unchanged for body, ~~JetBrains Mono for all metadata~~ — JetBrains Mono for metadata whose advance is load-bearing: the `tabular-nums` date columns, technical strings, badges, the CV contact block. Fraunces for wordmark, hero, essay/role/project titles; italic is the emphasis device. Mono metadata against serif display is what keeps the layout from reading as a blog template.

- Section headings: **15px, 600, uppercase, `tracking-[0.18em]`, accent, hairline rule** — ~~mono~~, the label face since ADR-0006. 13px was measured at 0.43× the smallest `h3` it headed and was the lowest-contrast text on the page.
...
- Discoverability: ~~mono~~ (label-face since ADR-0006) `CV →` in the sticky rail, repeated in the footer, and in the masthead route links from `lg`. ~~command-menu **Print** becomes context-dependent (**View CV** off-route, **Print CV** on-route)~~ — the palette carried that pair and retired with it in #89; off-route the CV is a visible link, on-route printing is the document's own `Print CV` button.
...
- **ATS extraction must be verified, not assumed.** A two-column grid with subsetted variable Fraunces and `tracking-[0.12em]` uppercase mono is a known mangling combination. **The uppercase term changed face in ADR-0006, and re-running the extraction caught real damage: at `tracking-[0.18em]` Space Grotesk extracted `PROJECTS` and `EDUCATION` as `P R O J E CTS` and `E D U CAT I O N`. The CV headings carry `print:tracking-[0.08em]` for that reason. Re-run the extraction on any further label-typography change; do not inherit this result either.** The generated PDF collapses to a single column, and the acceptance test extracts its text.
...
**Count-aware rendering.** The publication does not exist yet and its launch is coupled to the site's, so **day one is exactly one post**. 0 → section absent entirely. 1 → lead treatment only, no archive link. 2–3 → lead + compact rows. 4+ → full index and a "Read all essays" link to **`${SUBSTACK_BASE}/archive`** (an off-site Substack URL — there is no local `/archive` route and none is planned). The section must never imply more writing exists than does. At n=1 a **launch line** renders (label face since ADR-0006) ("First essay published \<date\> · new ones roughly fortnightly") — without it the page read as _a page with one thing on it_ under a standfirst promising "essays" plural.
...
- **Manual, once per size:** a visual read of the rendered PDF for typographic damage that geometry checks cannot see (broken ligatures, label and mono tracking collapse, italic substitution).
...
| ATS mangles the generated PDF                                     | **Measured risk factors** — subsetted variable Fraunces + `tracking-[0.12em]` uppercase label face (mono when measured; ADR-0006) + two-column grid is a known mangling combination | Medium                           | Single-column collapse is specified, and `pdftotext` extraction is an acceptance test rather than an assumption                                                                                           |
