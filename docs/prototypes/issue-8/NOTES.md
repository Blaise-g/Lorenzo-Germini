...
- **Mobile serif sizing** — confirmed, and worse than recorded. The hero switches to `md:text-6xl` at the _same_ breakpoint that turns on the 240px rail, leaving a 384px main column at 768px — 9–10 lines for a 22-word sentence. Below `md` the rail stacks first, pushing the `<h1>` below the fold. Now owned by #12; fix is `clamp(2rem, 5.2vw, 3.75rem)` and moving the rail to `lg:`.
